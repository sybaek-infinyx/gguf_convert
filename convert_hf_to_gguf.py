...
class Model:
    ...
    def __init__(self, dir_model: Path, ftype: gguf.LlamaFileType, fname_out: Path, is_big_endian: bool = False,
        ...
        self.gguf_writer_projector = gguf.GGUFWriter(path=None, arch=gguf.MODEL_ARCH_NAMES[self.model_arch],
    ...
    def get_tensors(self) -> Iterator[tuple[str, Tensor]]:
        ...
        tensor_seq = OrderedDict()

        if index_file.is_file():
            self.tensor_names = set()
            logger.info(f"gguf: loading model weight map from '{index_name}'")
            with open(index_file, "r", encoding="utf-8") as f:
                index: dict[str, Any] = json.load(f, object_pairs_hook=OrderedDict)
                weight_map = index.get("weight_map")
                if weight_map is None or not isinstance(weight_map, dict):
                    raise ValueError(f"Can't load 'weight_map' from {index_name!r}")
                self.tensor_names.update(weight_map.keys())
                tensor_seq = weight_map
        else:
            self.tensor_names = tensor_names_from_parts
            weight_map = {}

        # logging.info(f"tensor===>{tensor_seq}")
        # exit()
        ...
        for part_name in self.part_names:
            ...
            # mm이 맨마지막 파일에 있어서 정렬했는데 사실 할 필요 없더라...
            with ctx as model_part:
                ...
                for name in model_part.keys():
                    ...
                    tensor_seq[name] = data

        # logging.info(f"tensor===>{tensor_seq}")
        # exit()

        for name, data in tensor_seq.items():
            if isinstance(data, str):
                # raise ValueError(f"what tensor: {name, data}")
                continue
    ...
    def set_gguf_parameters(self, gguf_writer):
        gguf_writer.add_block_count(self.block_count)
        ...
        if (f_rms_eps := self.hparams.get("rms_norm_eps")) is not None:
            gguf_writer.add_layer_norm_rms_eps(f_rms_eps)
            logger.info(f"gguf: rms norm epsilon = {f_rms_eps}")
        if (f_norm_eps := self.find_hparam(["layer_norm_eps", "layer_norm_epsilon", "norm_epsilon"], optional=True)) is not None:
            gguf_writer.add_layer_norm_eps(f_norm_eps)
            logger.info(f"gguf: layer norm epsilon = {f_norm_eps}")
        if (n_experts := self.hparams.get("num_local_experts")) is not None:
            gguf_writer.add_expert_count(n_experts)
            logger.info(f"gguf: expert count = {n_experts}")
        if (n_experts_used := self.hparams.get("num_experts_per_tok")) is not None:
            gguf_writer.add_expert_used_count(n_experts_used)
    ...
    def prepare_tensors(self):
        ...
        # tmp_list = [(self.format_tensor_name(gguf.MODEL_TENSOR.MM_V_TILE_POS_GATE, suffix=''), torch.tensor([-0.868790328502655], dtype=torch.float32))]
        for name, data_torch in chain(self.generate_extra_tensors(), self.get_tensors()): #, tmp_list):
            # we don't need these
            ...
            for new_name, data_torch in (self.modify_tensors(data_torch, name, bid)):
                ...
                if data_qtype is False and (
                    any(
                        ...
                        for key in (
                            ...
                            gguf.MODEL_TENSOR.MM_V_PATCH_EMBD,
                            gguf.MODEL_TENSOR.MM_V_TILE_POS_EMBD,
                            gguf.MODEL_TENSOR.MM_PROJECTOR,
                            gguf.MODEL_TENSOR.MM_V_PRE_TILE_POS_EMBD,
                            gguf.MODEL_TENSOR.MM_V_PRE_TILE_POS_GATE,
                            gguf.MODEL_TENSOR.MM_V_POST_TILE_POS_EMBD,
                            gguf.MODEL_TENSOR.MM_V_POST_TILE_POS_GATE,
                            gguf.MODEL_TENSOR.MM_V_INPUT_NORM,
                            gguf.MODEL_TENSOR.MM_V_ATTN_OUT_NORM,
                            gguf.MODEL_TENSOR.MM_V_GLOBAL_ATTN_GATE,
                            gguf.MODEL_TENSOR.MM_V_GLOBAL_FFN_GATE,
                            gguf.MODEL_TENSOR.MM_V_GLOBAL_INPUT_NORM,
                            gguf.MODEL_TENSOR.MM_V_GLOBAL_POST_ATTN_NORM,
                            gguf.MODEL_TENSOR.MM_V_POS_GATE,
                            gguf.MODEL_TENSOR.MM_V_TILE_POS_GATE,
                            gguf.MODEL_TENSOR.MM_V_POST_NORM,
                            gguf.MODEL_TENSOR.MM_V_PRE_NORM,
                            ...
                            gguf.MODEL_TENSOR.MM_CROSS_ATTN_K_NORM,
                            gguf.MODEL_TENSOR.MM_CROSS_ATTN_Q_NORM,
                            gguf.MODEL_TENSOR.MM_CROSS_ATTN_GATE,
                            gguf.MODEL_TENSOR.MM_CROSS_ATTN_FFN_GATE,
                            gguf.MODEL_TENSOR.MM_CROSS_ATTN_NORM,
                        )
                    )
                ...
                if "mm.0.weight" in new_name:
                    data_qtype = gguf.GGMLQuantizationType.F16

                if "v.position_embd" in new_name and "gate" not in new_name:
                    data_qtype = gguf.GGMLQuantizationType.F16

                if "v.patch_embd" in new_name:
                    data_qtype = gguf.GGMLQuantizationType.F16
                ...
                # vision 쪽 걸러내기
                if new_name.startswith('v.') or new_name.startswith('mm.'):
                    # n_dims is implicit in the shape
                    logger.info("=========vision===========")
                    logger.info(
                        f"{f'%-{max_name_len}s' % f'{new_name},'} {old_dtype} --> {data_qtype.name}, shape = {shape_str}")

                    # v.patch_embd.weight v.position_embd
                    if "v.position_embd" in new_name and "gate" not in new_name:
                        new_name =  new_name + ".weight"

                    # if "mm.0" not in new_name:
                    #     data = data.transpose()

                    self.gguf_writer_projector.add_tensor(new_name, data, raw_dtype=data_qtype)
                else:
                    logger.info("=========text===========")
                    # n_dims is implicit in the shape
    ...
    def set_type(self, i):
        if i == 0:
            ...
            self.gguf_writer_projector.add_type("projector")

    def prepare_metadata(self, vocab_only: bool):
        # text, vision 총 2번 해야됨
        for i in range(2):
            # text 면:
            if i == 0:
                ...
                # vision 이면
                total_params, shared_params, expert_params, expert_count = self.gguf_writer_projector.get_total_parameter_count()
            ...
            if self.fname_out.is_dir():
                ...
                if i == 0:
                    ...
                    self.fname_out_projector = self.fname_out / f"{fname_default}_projector.gguf"
            else:
                ...
                if i == 0:
                    ...
                    self.fname_out_projector = self.fname_out.parent / gguf.fill_templated_filename(self.fname_out.name.replace(".gguf", "_projector.gguf"), output_type)

            logging.info(
                f"{self.metadata_override}, {self.dir_model_card}, {self.model_name}, {total_params}, {shared_params}, {expert_params}, {expert_count}, {self.metadata}, {self.fname_out}")
            ...
            if i == 0:
                ...
                self.metadata.set_gguf_meta_model_vision(self.gguf_writer_projector)

                # logger.info("Set model quantization version")
                # self.gguf_writer_projector.add_quantization_version(gguf.GGML_QUANT_VERSION)
    ...
    def write(self):
        ...
        self.gguf_writer_projector.write_header_to_file(path=self.fname_out_projector)
        self.gguf_writer_projector.write_kv_data_to_file()
        self.gguf_writer_projector.write_tensors_to_file(progress=True)

        self.gguf_writer.close()
        self.gguf_writer_projector.close()
    ...
    def load_hparams(dir_model: Path):
        with open(dir_model / "config.json", "r", encoding="utf-8") as f:
            ...
            if "text_config" in hparams:
                ...
                if "_name_or_path" in text_config and text_config["_name_or_path"].strip():
    ...
    def set_gguf_parameters(self, gguf_writer):
        ...
        if self.hparams.get("rope_scaling") is not None and "factor" in self.hparams["rope_scaling"]:
            if self.hparams["rope_scaling"].get("type") == "linear":
                ...
                gguf_writer.add_rope_scaling_factor(self.hparams["rope_scaling"]["factor"])
        ...
        # Apply to granite small models only
        if self.hparams.get("vocab_size", 32000) == 49152:
            gguf_writer.add_add_bos_token(False)

        # For vision model
        if self.vparams is not None and self.preprocessor_config is not None:
            gguf_writer.add_vision_type("clip-vit")
            gguf_writer.add_vision_image_size(self.vparams["image_size"])
            gguf_writer.add_vision_patch_size(self.vparams["patch_size"])
            gguf_writer.add_vision_clip_architecture("llava")
            gguf_writer.add_vision_clip_block_count(self.vparams["num_hidden_layers"])
            gguf_writer.add_vision_clip_embedding_length(self.vparams["hidden_size"])
            gguf_writer.add_vision_clip_feed_forward_length(self.vparams["intermediate_size"])
            gguf_writer.add_vision_clip_head_count(self.vparams["num_attention_heads"])
            gguf_writer.add_vision_clip_image_mean(self.preprocessor_config["image_mean"])
            gguf_writer.add_vision_clip_image_std(self.preprocessor_config["image_std"])
            gguf_writer.add_vision_clip_select_layer(self.hparams["vision_feature_layer"])
            gguf_writer.add_vision_clip_patch_merge_type(gguf.CLIPPatchMergeType.FLAT)
            max_pos_embd = (self.vparams["image_size"] // self.vparams["patch_size"])**2 + 1
            gguf_writer.add_vision_clip_max_position_embeddings(max_pos_embd)
            # TODO: should not hardcode these, but they are currently missing from config.json
            gguf_writer.add_vision_clip_projector_type(gguf.constants.CLIPProjectorType.MLP)
            gguf_writer.add_vision_clip_layer_norm_epsilon(1e-05)
...
class MLlamaModel(Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #tokenizer = AutoTokenizer.from_pretrained(self.dir_model)
        #vocab_size = len(tokenizer.vocab)
        self.text_config = hparams.get("text_config", {})
        #self.text_config['vocab_size'] = vocab_size
        #hparams['vocab_size'] = vocab_size
        self.vision_config = hparams.get("vision_config", {})

        # vision 저장 테스트용
        self.fname_out_projector=None
    ...
    def find_hparam(self, keys: Iterable[str], optional: bool = False) -> Any:
        ...
        if optional:
            return None
        raise KeyError(f"could not find any of: {keys} in self.text_config")

    def set_gguf_parameters(self):
        n_embd = self.text_config["hidden_size"]
        n_head = self.text_config["num_attention_heads"]

        image_token_index = self.find_hparam(["image_token_index"], optional=False)
        self.gguf_writer.add_image_token_index(image_token_index)

        ### Text model metadata
        self.gguf_writer.add_context_length(self.text_config["max_position_embeddings"])
        self.gguf_writer.add_block_count(self.text_config["num_hidden_layers"])
        self.gguf_writer.add_embedding_length(n_embd)
        self.gguf_writer.add_feed_forward_length(self.text_config["intermediate_size"])
        self.gguf_writer.add_head_count(n_head)
        self.gguf_writer.add_head_count_kv(self.text_config["num_key_value_heads"])
        self.gguf_writer.add_rope_freq_base(self.text_config["rope_theta"])
        self.gguf_writer.add_layer_norm_rms_eps(self.text_config["rms_norm_eps"])
        self.gguf_writer.add_file_type(self.ftype)
        self.gguf_writer.add_cross_attention_layers(self.text_config["cross_attention_layers"])
        self.gguf_writer.add_vocab_size(self.text_config["vocab_size"])
        self.gguf_writer.add_rope_dimension_count(n_embd // n_head)
        #self.gguf_writer.add_activation_function(self.text_config["hidden_act"])

        ### Vision metadata
        # TODO(danbev): cross-attn is to indicate that this vision model uses
        # cross-attention with the text model as opposed to the "prompt-based"
        # type of model where the image patch embeddings are returned to
        # caller and then passed to decode as normal tokens in the prompt.
        self.gguf_writer_projector.add_file_type(self.ftype)
        # self.gguf_writer_projector.add_vision_type("cross-attn")
        # self.gguf_writer_projector.add_vision_architecture(self.vision_config["model_type"])
        self.gguf_writer_projector.add_vision_image_size(self.vision_config["image_size"])
        self.gguf_writer_projector.add_vision_block_count(self.vision_config["num_hidden_layers"])
        self.gguf_writer_projector.add_vision_layer_norm_rms_eps(self.vision_config["norm_eps"])
        self.gguf_writer_projector.add_vision_embedding_length(self.vision_config["hidden_size"])
        # self.gguf_writer_projector.add_vision_activation_function(self.vision_config["hidden_act"])
        self.gguf_writer_projector.add_vision_feed_forward_length(self.vision_config["intermediate_size"])
        self.gguf_writer_projector.add_vision_global_block_count(self.vision_config["num_global_layers"])
        self.gguf_writer_projector.add_vision_max_num_tiles(self.vision_config["max_num_tiles"])
        self.gguf_writer_projector.add_vision_channels_count(self.vision_config["num_channels"])
        self.gguf_writer_projector.add_vision_patch_size(self.vision_config["patch_size"])
        self.gguf_writer_projector.add_vision_intermediate_layer_indices(self.vision_config["intermediate_layers_indices"])
        self.gguf_writer_projector.add_vision_attention_head_count(self.vision_config["attention_heads"])
        # self.gguf_writer_projector.add_vision_output_dimension(self.vision_config["vision_output_dim"])
        # self.gguf_writer_projector.add_vision_model_type(self.vision_config["model_type"])
        #
        # max_pos_embd = (self.vision_config["image_size"] // self.vision_config["patch_size"])**2 + 1
        # self.gguf_writer_projector.add_vision_clip_max_position_embeddings(max_pos_embd)
        # self.gguf_writer_projector.add_vision_supported_aspect_ratios(self.vision_config["supported_aspect_ratios"])
        # if self.preprocessor_config is not None:
        #     self.gguf_writer_projector.add_vision_clip_image_mean(self.preprocessor_config["image_mean"])
        #     self.gguf_writer_projector.add_vision_clip_image_std(self.preprocessor_config["image_std"])
        # else:
        #     raise ValueError("preprocessor_config is None, cannot access 'image_mean' or 'image_std'")
        self.gguf_writer_projector.add_vision_clip_projection_dimension(self.vision_config["vision_output_dim"])
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if name.startswith('language_model') and name.endswith(("k_proj.weight", "k_proj.bias")):
            print(f'[danbev] Permuting Language {name}')
            data_torch = LlamaModel.permute(data_torch, n_head, n_kv_head)

        if name.startswith('language_model') and name.endswith(("q_proj.weight", "q_proj.bias")):
            print(f'[danbev] Permuting Language {name}')
            data_torch = LlamaModel.permute(data_torch, n_head, n_head)

        # Just making the values exactly what ollama uses for testing and
        # debugging purposes.
        if name == "vision_model.pre_tile_positional_embedding.gate":
            print("vision_model.pre_tile_positional_embedding.gate")
            data_torch = data_torch.fill_(0.6351490020751953)

        if name == "vision_model.post_tile_positional_embedding.gate":
            print("vision_model.post_tile_positional_embedding.gate")
            data_torch = data_torch.fill_(-0.19474606215953827)

        if name == "vision_model.gated_positional_embedding.gate":
            print("vision_model.gated_positional_embedding.gate")
            data_torch = data_torch.fill_(1.8687903881072998)

        return [(self.map_tensor_name(name), data_torch)]

    def generate_extra_tensors(self) -> Iterable[tuple[str, Tensor]]:
        if rope_scaling := self.text_config.get("rope_scaling"):
            if rope_scaling.get("rope_type", '').lower() == "llama3":
               base = self.text_config.get("rope_theta", 500000.0)
               dim = self.text_config.get("head_dim", self.text_config["hidden_size"] // self.text_config["num_attention_heads"])
               freqs = 1.0 / (base ** (torch.arange(0, dim, 2, dtype=torch.float32) / dim))
        ...
        tile_position_embd_gate = torch.tensor([-0.868790328502655], dtype=torch.float32)
        yield (self.format_tensor_name(gguf.MODEL_TENSOR.MM_V_TILE_POS_GATE, suffix=''), tile_position_embd_gate)
    ...
    def set_vocab(self):
        ...
        special_vocab = gguf.SpecialVocab(self.dir_model, load_merges=True)
        special_vocab._set_special_token("start_header",        128006)  # <|start_header_id|>
        special_vocab._set_special_token("end_header",          128007)  # <|end_header_id|>
        special_vocab._set_special_token("finetune_right_pad_id",  128004)  # <|finetune_right_pad_id|>
        special_vocab._set_special_token("step",                   128005)  # <|step_id|>
        special_vocab._set_special_token("eom",                    128008)  # <|eom_id|>
        special_vocab._set_special_token("eot",                    128009)  # <|eot_id|>
        special_vocab._set_special_token("python_tag",             128010)  # <|python_tag|>
        special_vocab._set_special_token("image",                  128256)  # <|image|>
        special_vocab.add_to_gguf(self.gguf_writer)
