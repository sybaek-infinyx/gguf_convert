...
class GGUFWriter:
    ...
    def add_image_token_id(self, id: int) -> None:
        self.add_uint32(Keys.Tokenizer.IMAGE_ID, id)

    def add_start_header_token_id(self, id: int) -> None:
        self.add_uint32(Keys.Tokenizer.START_HEADER_ID, id)

    def add_end_header_token_id(self, id: int) -> None:
        self.add_uint32(Keys.Tokenizer.END_HEADER_ID, id)

    def add_python_tag_token_id(self, id: int) -> None:
        self.add_uint32(Keys.Tokenizer.PYTHON_TAG_ID, id)
    ...
    def add_vision_clip_projection_dimension(self, dim: int) -> None:
        self.add_uint32(Keys.Vision.Clip.PROJECTION_DIM, dim)
    ...
    # Multi-Modal Vision methods. TODO: consolidate with Vision methods
    def add_image_token_index(self, idx: int) -> None:
        self.add_uint32(Keys.LLM.IMAGE_TOKEN_INDEX.format(arch=self.arch), idx)

    def add_cross_attention_layers(self, layers: Sequence[int]) -> None:
        self.add_array(Keys.LLM.CROSS_ATTENTION_LAYERS.format(arch=self.arch), layers)

    def add_vision_block_count(self, heads: int) -> None:
        self.add_uint32(Keys.Vision.BLOCK_COUNT, heads)

    def add_vision_layer_norm_rms_eps(self, value: float) -> None:
        self.add_float32(Keys.Vision.LAYERNORM_RMS_EPS, value)

    def add_vision_embedding_length(self, size: int) -> None:
        self.add_uint32(Keys.Vision.EMBEDDING_LENGTH, size)

    def add_vision_feed_forward_length(self, length: int) -> None:
        self.add_uint32(Keys.Vision.FEED_FORWARD_LENGTH, length)

    def add_vision_global_block_count(self, size: int) -> None:
        self.add_uint32(Keys.Vision.Cross.GLOBAL_BLOCK_COUNT.format(arch=self.arch), size)

    def add_vision_max_num_tiles(self, value: int) -> None:
        self.add_uint32(Keys.Vision.Cross.MAX_NUM_TILES.format(arch=self.arch), value)

    def add_vision_channels_count(self, channels: int) -> None:
        self.add_uint32(Keys.Vision.Cross.CHANNELS_COUNT.format(arch=self.arch), channels)

    def add_vision_intermediate_layer_indices(self, indices: Sequence[int]) -> None:
        self.add_array(Keys.Vision.Cross.INTERMEDIATE_LAYERS_INDICES.format(arch=self.arch), indices)

    def add_vision_attention_head_count(self, count: int) -> None:
        self.add_uint32(Keys.Vision.HEAD_COUNT, count)

    def add_vision_output_dimension(self, dim: int) -> None:
        self.add_uint32(Keys.Vision.Cross.OUTPUT_DIM.format(arch=self.arch), dim)

    def add_vision_supported_aspect_ratios(self, ratios: Sequence[Sequence[int]]) -> None:
        flattened_ratios = [ratio for sequence in ratios for ratio in sequence]
        self.add_array(Keys.Vision.Cross.SUPPORTED_ASPECT_RATIOS.format(arch=self.arch), flattened_ratios)

    def add_vision_model_type(self, model_type: str) -> None:
        self.add_string(Keys.Vision.Cross.MODEL_TYPE.format(arch=self.arch), model_type)

    def add_vision_activation_function(self, act: str) -> None:
        self.add_string(Keys.Vision.Cross.ACTIVATION_FUNCTION.format(arch=self.arch), act)

    def add_vision_architecture(self, value: str) -> None:
        self.add_string(Keys.Vision.ARCHITECTURE, value)
